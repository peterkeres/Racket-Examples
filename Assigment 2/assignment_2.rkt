#lang racket 
;#lang plai
; assigment 2 algrithims
; peter keres
; sep 7 2019

; multiple 2 bit list together following the algrithim in figure 1.1
; example (multiply (list 100101010) (list 11111000))
(define multiply
  (lambda (bitx bity)
    (cond
      ;if the y bit is 0, return 0
      [ (zero? bity)  0]
      ; else, we need to half the bitlist of y by popping off the front bit
      [ else  (let [(bitz (multiply bitx (cdr bity)))]
                (cond
                  ;if its even double it, by adding a 0 to the bit list
                  [(= 0 (car bity))  (cons 0 bitz)]
                  ; if its odd we double it then add in bitx
                  [ else     (add bitx (cons 0 bitz))]
                  )
                )
              ]
      )
